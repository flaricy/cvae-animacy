...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.cfg.train.epochs):
            ...
            for batch_count, data in enumerate(policy_model_dataloader, start=1):
                ...
                generate_state = torch.zeros_like(data['gt_state'], device=self.cfg.device)
                ...
                for t in range(data['gt_state'].shape[1] - 1):
                    latent_t, generate_delta_latent[:, t] = self.model.posterior(generate_state[:, t], data['gt_state'][:, t + 1], return_delta=True)
                    action_t = self.model.policy(generate_state[:, t], latent_t)
                    generate_state[:, t + 1] = self.model.world_model(generate_state[:, t], action_t)
    ...
    def _prepare_model(self):
        ...
        self.scheduler = build_scheduler(self.cfg.train.scheduler, self.world_model_optimizer)
